...
export const AdminLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans overflow-hidden relative">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 w-72 bg-[#0F172A] border-r border-slate-800 flex flex-col shadow-xl z-30 transition-transform duration-300 md:relative md:translate-x-0 ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        ...
        <div className="p-4 border-t border-slate-800 mt-auto">
          <div className="flex items-center gap-3 mb-4 px-2">
            ...
            <div className="text-sm truncate text-slate-300 flex-1">
              {user?.email || "nurbeksaliyev08@gmail.com"}
            </div>
          </div>
          <button
            onClick={signOut}
