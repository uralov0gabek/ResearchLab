...
// ---------------------------------------------------------
// SECTION B: Formative years (ages 15-24)
// ---------------------------------------------------------
...
// ---------------------------------------------------------
// CPT TASKS: Gain Lotteries (Block 1) - alpha
// ---------------------------------------------------------
const G1_SURE_AMOUNTS = [200000, 400000, 600000, 800000, 1000000, 1200000];
G1_SURE_AMOUNTS.forEach((sure, i) => addCPT("G1", `G1${String.fromCharCode(97+i)}`, sure, 1500000, 50, 0, 50, `11111111-2222-3333-4444-10000000000${i}`));

const G2_SURE_AMOUNTS = [100000, 200000, 300000, 400000, 500000, 550000];
G2_SURE_AMOUNTS.forEach((sure, i) => addCPT("G2", `G2${String.fromCharCode(97+i)}`, sure, 600000, 50, 0, 50, `11111111-2222-3333-4444-10000000001${i}`));

const G3_SURE_AMOUNTS = [400000, 800000, 1200000, 1600000, 2000000, 2400000];
G3_SURE_AMOUNTS.forEach((sure, i) => addCPT("G3", `G3${String.fromCharCode(97+i)}`, sure, 3000000, 50, 0, 50, `11111111-2222-3333-4444-10000000002${i}`));

// ---------------------------------------------------------
// CPT TASKS: Loss Lotteries (Block 2) - beta
// ---------------------------------------------------------
const L1_SURE_AMOUNTS = [-200000, -400000, -600000, -800000, -1000000, -1200000];
L1_SURE_AMOUNTS.forEach((sure, i) => addCPT("L1", `L1${String.fromCharCode(97+i)}`, sure, -1500000, 50, 0, 50, `11111111-2222-3333-4444-20000000000${i}`));

const L2_SURE_AMOUNTS = [-100000, -200000, -300000, -400000, -500000, -550000];
L2_SURE_AMOUNTS.forEach((sure, i) => addCPT("L2", `L2${String.fromCharCode(97+i)}`, sure, -600000, 50, 0, 50, `11111111-2222-3333-4444-20000000001${i}`));

const L3_SURE_AMOUNTS = [-400000, -800000, -1200000, -1600000, -2000000, -2400000];
L3_SURE_AMOUNTS.forEach((sure, i) => addCPT("L3", `L3${String.fromCharCode(97+i)}`, sure, -3000000, 50, 0, 50, `11111111-2222-3333-4444-20000000002${i}`));

// ---------------------------------------------------------
// CPT TASKS: Mixed Lotteries (Block 3) - lambda
// ---------------------------------------------------------
const M1_WIN_AMOUNTS = [400000, 600000, 800000, 1000000, 1200000];
M1_WIN_AMOUNTS.forEach((win, i) => addCPT("M1", `M1${String.fromCharCode(97+i)}`, 0, win, 50, -500000, 50, `11111111-2222-3333-4444-30000000000${i}`));

const M2_WIN_AMOUNTS = [600000, 800000, 1000000, 1400000, 1800000];
M2_WIN_AMOUNTS.forEach((win, i) => addCPT("M2", `M2${String.fromCharCode(97+i)}`, 0, win, 50, -1000000, 50, `11111111-2222-3333-4444-30000000001${i}`));

const M3_WIN_AMOUNTS = [150000, 250000, 350000, 500000];
M3_WIN_AMOUNTS.forEach((win, i) => addCPT("M3", `M3${String.fromCharCode(97+i)}`, 0, win, 50, -200000, 50, `11111111-2222-3333-4444-30000000002${i}`));

// ---------------------------------------------------------
// SECTION F: UzCombinator - motivation and exit plans
// ---------------------------------------------------------
...
// ---------------------------------------------------------
// SECTION G: Perceptions of accepted founders
// ---------------------------------------------------------
// Used for UzCombinator staff, mentors, founders. Just basic general logic.
...
async function runSeed() {
  ...
  await supabase.from('cpt_tasks').delete().neq('id', '00000000-0000-0000-0000-000000000000'); 
  await supabase.from('survey_modules').delete().neq('id', '00000000-0000-0000-0000-000000000000'); 

  console.log(`🌱 Writing ${questions.length} regular questions...`);
  for (let i = 0; i < questions.length; i += 10) {
    const chunk = questions.slice(i, i + 10);
    const { error } = await supabase.from('questions').upsert(chunk);
    if (error) {
      console.error(`❌ Error inserting questions chunk ${i}:`, error.message);
      return;
    }
  }

  console.log(`🌱 Writing ${cptTasks.length} CPT tasks...`);
  for (let i = 0; i < cptTasks.length; i += 10) {
    const chunk = cptTasks.slice(i, i + 10);
    const { error } = await supabase.from('cpt_tasks').upsert(chunk);
    if (error) {
      console.error(`❌ Error inserting CPT tasks chunk ${i}:`, error.message);
      return;
    }
  }

  console.log(`✅ All questions and CPT tasks saved successfully! Ready for production!`);
}
